...
TEST_CASE("Test Results class") {
    SECTION("It cannot store old results when use same Results object multiple times") {
        TestSetup setup;

        sqlite::exec(setup.db, "INSERT INTO Customers (id, name, age) VALUES (1, 'Marshall', 21);");
        sqlite::exec(setup.db, "INSERT INTO Customers (id, name, age) VALUES (2, 'John', 22);");
        sqlite::exec(setup.db, "INSERT INTO Customers (id, name, age) VALUES (1, 'Ben', 45);");

        sqlite::Results results;
        size_t expected_results_count = 3;
        sqlite::exec(setup.db, "SELECT * FROM Customers;", results);
        REQUIRE(results.count() == expected_results_count);

        sqlite::exec(setup.db, "DROP TABLE IF EXISTS Customers;");
